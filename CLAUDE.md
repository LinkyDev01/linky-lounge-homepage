...
| `bookclub_class_assignments` (0015) | `/classes` 열람 페이지만 (`lib/class-assignments.ts`) | **레포가 public 이라 명단은 DB 에만** — 컨피그·마이그레이션에 이름을 적지 않는다. 시드는 SQL 로 직접. 암호는 env `BOOKCLUB_CLASSES_PASSWORD`(없으면 닫힘). 이름 파기 `purge_expired_class_assignments()` — cron 잡이 세 함수를 부른다 |
