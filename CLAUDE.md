...
- **기수 전환 시**: season-config.ts + 이 문서·DECISIONS 의 기수 표기 갱신. **현: 5기 `status:"upcoming"`** — 일정(11/4–12/27, 화·수·토·일)은 공개하되 **접수는 안 열림**(운영자 2026-09-09 "일정만 노출, 접수는 알림받기 유지"). `/apply` 는 2겹 가드로 닫혀 있고 CTA·내비는 '5기 오픈 알림'. 4기(9/9–11/1)는 **모임 진행 중**이라 레이지클럽 지난 기수 진열·캘린더에 상수로 남겼다(`WorkroomHome.PAST_SEASONS`·`calendar-events.SEASON4_SESSIONS`). **5기 책 4권은 원고·표지 대기**(운영자 "이미지와 내용 주면 넣어") — `book-config` 는 4기가 최신이고, 그래서 `BookSection.isCurrent` 는 최신 탭이 곧 `SEASON.name` 일 때만 참이다.
