...
- **프리뷰 트리** `app/(main)/lazyday/preview/` : noindex + PreviewBar + 자체 폰트 로드(layout.tsx). 실사이트 컴포넌트의 V2 대응물 + 프리뷰 전용(PhilosophySectionV2, faq-designs). **2026-08-24 아키텍처 재동기화** — 셸(내비·푸터·진행순서·후기·클로징)이 실사이트 것을 **직접 import**(V2 없음): `LandingShell`·`HeroParallax`·`HowToBrief`·`ReviewsSection`·`SeasonCountCta`+`BrandCloseV2`. Schedule·모임소개(FeatureQuietSection)만 실↔프리뷰 **분리 사본 쌍**이 유효(§4 참조 — 한쪽 수정 시 쌍도 같은 값으로). 구 `NavBarV2`·`Footer`(구 프리뷰 전용)·`HowToSectionV2`·`ClosingSectionV2`·구 `preview/ReviewsSection`(실물 사진 대기 시절 플레이스홀더)는 실사이트의 대응 고아들과 함께 **고아 보존**(렌더 안 함, 삭제 금지). FifthSession은 섹션 삭제로 실·프리뷰 모두 미렌더(고아 보존). **데스크톱(≥721px) 반응형 훅도 포트 완료**(2026-08-24) — Book/Schedule/FeatureQuietSection(프리뷰)는 자체 CSS 파일(`preview.module.css`·`ScheduleSectionV2.module.css`·`preview/FeatureQuietSection.module.css`)에서 `landing-shell.module.css`가 주입하는 `--lz-carousel-*`·`--lz-cover-*`·`--lz-cal-*`·`--lz-qt-*`·`--lz-rule` 변수를 실사이트와 동일하게 소비하도록 값을 맞췄다(390·1280px 픽셀 동일 실측) — **`--lz-*` 변수를 새로 추가·변경할 땐 이 3개 프리뷰 파일도 같이 확인할 것**(안 하면 프리뷰만 조용히 데스크톱 확장에서 빠진다). **배경 교차도 실사이트와 동일하게 재배정 완료**(2026-08-24, 운영자 확인) — Schedule A→B. 예외 1건: `ScenesSection`(사진 콜라주, 프리뷰 전용 보류, 배경 A)이 후기(A)와 FAQ(B) 사이에 끼어 있는데, 이미 교차 중인 두 색 사이에 실사이트에 없는 섹션 하나를 끼우는 이상 **2색 교차로는 완전한 해소가 수학적으로 불가능**(홀수 삽입) — 나머지 전 구간을 실사이트와 맞추는 대신 후기(A)-Scenes(A) 인접 하나만 예외로 남겼다(선례: 2026-07-22~08-17 후기-FAQ B-B 인접도 같은 방식으로 용인). Scenes 실사이트 이식(사진 확보) 시 재검토.
...
| `FifthSessionSection`·`LandingShell`(landing-shell.module.css)·`HowToBrief`(howto-brief.module.css)·`ReviewsSection`(ReviewsSection.module.css)·`SeasonCountCta`(SeasonCountCta.module.css 등)의 module.css | **V2 없음** — 프리뷰 랜딩이 실사이트 컴포넌트를 직접 import (2026-08-24부로 후자 4개 추가) | 수정 = 실사이트+프리뷰 동시 변경. 시안 실험 시 이 파일들 수정 금지, 쇼케이스 전용 CSS를 새로 만들 것 |
