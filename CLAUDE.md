...
| `applications.triage` (0008) | admin 화면이 유일한 소비자 | **파기가 아니라 열람 필터**(test·dummy·typo·duplicate·paid). 기본 목록은 `triage is null`, `?triaged=1` 로 꺼내 본다. ⚠ **진행 상태(status)와 섞지 말 것** — 그 정본은 시트, 여는 건 P5 |
