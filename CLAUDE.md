...
- **도메인**: lazyday-bookclub.com → 미들웨어가 `/lazyday/*` rewrite. `linkylounge.com/lazyday/*` 는 301. `/lazyday/admin*` 은 `lazyday_admin` 쿠키(=ADMIN_SECRET). **관리 화면은 북클럽 도메인 전용**(운영자 2026-09-01 유지 확정) — `lazy-club.com/admin*` 은 404 이고, 쿠키가 host-only 라 옮기면 도메인마다 재로그인이다.
