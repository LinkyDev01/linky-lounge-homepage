...
**진행 중 계획**: `docs/handoff/2026-09-02-dashboard-to-next-session.md`(**세션 인수인계 — 먼저 읽는다**: 현재 상태·다음 할 일 순서·운영자 대기) · `docs/admin-crm/02-build-plan.md`(대시보드 PR 순서표) · `docs/plans/2026-09-01-applications-ledger-and-members.md` — 접수 시트+DB 이중화 · 회원(profiles·소셜 로그인) · DB 관리 화면 (P0~P5, 단계별 독립 PR). 이 범위의 작업은 그 문서를 먼저 읽는다. · `docs/plans/2026-09-03-host-recruitment-page.md` — 모임장 모집(lazy-club.com `/hosts`) + 북클럽 원형 모임 페이지(`/meetings/lazyday-bookclub`) 기획 v3 — **레이지클럽은 모임장이 자기 모임을 직접 기획해 가져오고 협의로 확정하는 구조. 페이 미고지. 레퍼런스 참고 안 함**(운영자 2026-09-03). **안내 페이지+접수란 승인·병합됨**(운영자 2026-09-03 "승인할게") `app/(main)/lazyclub/hosts/`(lazy-club.com `/hosts` index·사이트맵, kind `host`·GAS `handleHost`, 방침 제1조 5호·제2조 6목·제3조 5호). **진입점(2026-09-10 확정)**: 사람 페이지 우측 sticky 카드 `people/HostsAside`(포스터 + '레이지클럽 / 호스트 모집 중', `/meetings` 분할 구도) — 내비·푸터 링크는 없음. **`/hosts` v3(2026-09-10)**: 모임 상세 구도(좌 sticky 포스터 `hosts-poster.webp` · 우 요약 `hosts-config.ts` HOSTS_FIELDS · 잉크 칩 '기획서 쓰기' · 하단 본문) — home.module.css 의 nsq·product 클래스를 소비만 한다. `/hosts/apply` 여섯 걸음 기획서(noindex, `HostPlanFlow`, 2번째 걸음은 진행 간격 1주/2주). **사용자 화면 표기는 '호스트'**(운영자 원문), 관리 라벨·시트·방침은 '모임장' 그대로. '레이지클럽'은 붙여 쓴다. 문체 원칙: 주어 '저희', 말하듯 쓴 제목, 동사가 있는 문장. ⚠ 북클럽 철학 4축·**'문학과 철학' 범위·브랜드 3문단은 전부 북클럽 것** — 레이지클럽 페이지에 쓰지 않는다(운영자 2026-09-03). 레이지클럽 카피 재료는 **트리에 있는 마케팅 문구만**(브랜드 문장·원데이 토크 설명·홈 도록체 캡션·랜딩 키워드) — 새 은유·새 어휘 금지, **짧게 끊어 단정하는 문장 금지**(원고처럼 길게 이어지다 초대로 맺는 호흡), **북클럽은 언급만 하고 소개하지 않는다**, 4권의 책 틀 없음(운영자 2026-09-03). 모임은 **총 4회차·각 2시간·1주 또는 2주 간격·장소 6곳 중 선택·주제 자유**이고 '책, 영화 등 매개체는 무엇이든'은 운영자 카드뉴스 원문이라 써도 된다(2026-09-10, 09-03 '책·영화 언급 없음'을 대체). 병합 전 방침 개정 선행. 나머지 결정은 계획서 §9·§12.
...
- **히어로** = `HeroSpinPoster`(5기 회전 포스터 — 두 장이 반대로 맞물려 돈다, HeroParallax 가 렌더. 원본은 `preview/poster-5th`, 값·근거는 그 CSS 머리말). **4기 `HeroBreathingPoster`(한붓 그리기)는 고아 보존 — 검수대 `preview/hero-check` 가 백업 창구다**(운영자 2026-09-09 "4기 애니메이션은 따로 백업"). 아래 진입 안무 설명은 그 4기 모션 기준(`useChromeIntro.HOLD_ENABLED=true`). 진입 안무: 포스터만 → 그어짐 끝 무렵 내비·푸터·본문 → +3초 스티키 CTA. 검수대 `preview/hero-check`. 포스터 내부 작업은 `docs/env-notes.md` 필독.
...
- **고아 보존**(렌더 안 함 — 삭제 금지): HeroBreathingPoster(4기 히어로, hero-check 에서만 렌더)·About·Closing·Rules·Vibe·FeatureBoxSection·FifthSessionSection·HowToSection·HowToBrief·NavBar·BrandCloseSection + 프리뷰 쪽 NavBarV2·Footer·HowToSectionV2·ClosingSectionV2·구 preview/ReviewsSection.
...
- **단일 출처 컨피그**: `season-config.ts`(기수·기간·마감·가격·회차·장소 — **기수 전환은 이 파일만**. `status` 4값: open·closedEarly(마감)·upcoming(오픈 전)·**locked(접수도 알림도 없음)**. 화면은 리터럴 비교 대신 파생 `NOTIFY_MODE`/`NOTIFY_SEASON`/`NOTIFY_SCHEDULE`/`LOCKED` 을 쓴다 — ⚠ `NOTIFY_MODE` 는 locked 를 **제외**한다(알림조차 안 받는 상태) — upcoming 에서 알림 폼이 안내하는 기수는 '다음'이 아니라 **이번** 기수다. 연도는 `year` 명시 필드 — deadline 이 null 이면 파기 기준일이 폴백으로 샜다) · `book-config.ts`(기수별 책 4권) · `philosophy-content.tsx`(확정 원고) · `preview/preview-config.ts`(파생) · `one-day-talk-01/oneday-shared.ts`(원데이 일정·가격·orderId 계약 — confirm 이 금액 재검증하므로 §4 데이터 지도 참조).
...
| `bookclub_class_assignments` (0015) · `bookclub_class_access` (0016) | `/classes` 열람 페이지만 (`lib/class-assignments.ts`·`lib/classes-gate.ts`) | **레포가 public 이라 명단도 암호도 DB 에만** — 컨피그·마이그레이션에 이름·암호를 적지 않는다(시드는 SQL 로 직접). 암호는 sha256(trim+대문자), 행이 없으면 닫힌다. env 가 아닌 이유: 운영자가 Vercel 에 넣고 재배포해야 열리는 것을 피한다. 이름 파기 `purge_expired_class_assignments()` — cron 잡이 세 함수를 부른다(암호 테이블은 개인정보가 아니라 파기 대상 아님) |
...
- **이 컨테이너의 `node_modules` 는 락과 어긋날 수 있다** — 2026-09-08 실측: `@supabase/ssr` 이 `package.json`·`package-lock.json` 에 다 있는데 **설치만 빠져** `npm run build` 와 `/api/lazyday/apply` 가 `Module not found` 로 깨졌다(내 변경 탓으로 오인하기 쉽다). **`Module not found` 는 코드가 아니라 설치 누락부터 의심**하고, `git stash` 로 main 도 같이 깨지는지 대조한 뒤 `npm install --no-save <pkg>` 로 채운다(매니페스트를 건드리지 않는다).
...
- **기수 전환 시**: season-config.ts + 이 문서·DECISIONS 의 기수 표기 갱신. **현: 5기 `status:"locked"`** — 일정(11/4–12/27, 화·수·토·일)·회전 포스터는 공개하되 **누를 수 있는 것이 하나도 없다**(운영자 2026-09-09 "일단 사람들에게 아무것도 못하게 할 거야"). `/apply` 2겹 가드 + 스티키 CTA 는 회색 죽은 `SEASON.lockedLabel`("4기 모집 마감" — ⚠ 기수 파생 금지, 5기는 모집을 연 적이 없다), 내비 CTA·알림 폼은 렌더 자체를 안 한다. 4기(9/9–11/1)는 **모임 진행 중**이라 레이지클럽 지난 기수 진열·캘린더에 상수로 남겼다(`WorkroomHome.PAST_SEASONS`·`calendar-events.SEASON4_SESSIONS`). **5기 책 4권은 원고·표지 대기**(운영자 "이미지와 내용 주면 넣어") — `book-config` 는 4기가 최신이고, 그래서 `BookSection.isCurrent` 는 최신 탭이 곧 `SEASON.name` 일 때만 참이다.
