...
- **실사이트 랜딩** `app/(main)/lazyday/page.tsx` 섹션 순서(2026-07-06 이식 후): Hero → **선정도서(BookSection)** → 5회차(FifthSession) → 모임소개(**FeatureQuietSection**, 콰이어트) → 진행방식(HowTo, 콰이어트) → 일정(Schedule, 콰이어트) → FAQ → **ClosingCta** → BrandClose. About/Closing/Rules/Vibe/**FeatureBoxSection** 섹션은 **고아 상태로 보존**(렌더 안 함 — 삭제하지 말 것).
...
| `HowToSection`·`ScheduleSection`·`FeatureQuietSection`·`ClosingCtaSection` (실) ↔ `HowToSectionV2`·`ScheduleSectionV2`·`preview/FeatureQuietSection`·`ClosingSectionV2` (프리뷰) | **분리된 파일 쌍** (2026-07-06 콰이어트 이식) | 실·프리뷰가 같은 디자인의 **별도 사본** — 한쪽 수정 시 반드시 쌍도 같은 값으로 (TSX 쌍 동기화 원칙). CSS import 공유 없음 |
