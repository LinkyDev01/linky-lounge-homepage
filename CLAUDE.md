...
- **검증 기준 뷰포트: 390px (기준) + 768/1280 (반응형)**. 모바일 퍼스트 — 스크린샷 검증은 3종으로.
- **반응형 원칙 — "요소를 키우는 반응형"이 아니라 "여백을 설계하는 반응형"**: 브레이크 3단(≤679 = 390 문법 그대로 / 680–1023 = 콘텐츠 칼럼 560 중앙 / ≥1024 = 데스크톱, 텍스트 칼럼 640 중앙 고정 — 폭을 늘리지 말 것). 2단 재배열은 3곳만(히어로·책 소개·SCENES 3열). 오브제(종이 낱장·일정 박스·테이프·손그림)는 고정폭 ~420px — 종이는 화면 따라 커지지 않는다. hover 어포던스는 @media(hover:hover)에서 들림만(호버 전용 정보 금지), fixed 요소에 모바일 좌표 하드코딩 금지.
