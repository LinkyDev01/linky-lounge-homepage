...
- **레이지클럽 경로 (2026-08-21 프리뷰 졸업)**: 트리가 `app/(main)/lazyclub/**` 로 이사했다 (구 `lazyday/preview/lazyclub-4b073000ddec094f`). 프리픽스 상수는 `lazyclub/base-path.ts` 의 `BASE = "/lazyclub"`, 홈은 `HOME = "/lazyclub/all"`. **URL 명명 규칙(2026-08-21 확정): 슬러그 = 내비 라벨의 영문 대응어** — 전체보기=`all` · 모임=`meetings` · 제품=`products` · 사람=`people` · 일정=`schedule` · 기록=`records`. 새 섹션을 만들 때 이 대응을 지킬 것(구 `shop`·`calendar`·`archive` 는 301 로만 남아 있다). **URL 전반의 규율(슬러그 불변·301 원장·개명 절차)은 `docs/url-policy.md` 가 정본**(2026-08-23 제정) — 슬러그를 새로 짓거나 바꾸기 전에 필독. ⚠ **홈은 트리 루트가 아니라 `/all`** — lazy-club.com 에서 프리픽스를 떼면 트리 루트가 `/`(랜딩=coming-soon)와 겹쳐 홈이 사라진다(실측 회귀). **lazy-club.com 은 이 트리가 도메인 루트** — 미들웨어가 `/meetings` → `/lazyclub/meetings` 로 rewrite 하고, 랜딩(`/`)만 `LAZYCLUB_LIVE=false` 동안 coming-soon. 북클럽 도메인에서는 `/lazyclub/*` 로 열린다(운영자 검토용, 레이아웃이 noindex). 구 경로 2종은 301. ⚠ 트리 안 내부 링크는 `LazydayLink` 가 아니라 **`LazyclubLink`** — `/lazyday` 프리픽스를 붙이면 없는 경로가 된다. ⚠ `lib/order-catalog.ts` 가 `lazyclub/goods-config` 를 import 한다 (프로덕션 결제 — §4 데이터 지도).
...
- **실사이트 랜딩** `app/(main)/lazyday/page.tsx` — **2026-08-12 반응형 개편 이식**: 페이지 전체가 `LandingShell`(레이지클럽 문법 내비+푸터, landing-shell.module.css)로 감싸이고, 내비는 동적 마크 `LazydayMark`(I♥LAZYDAY, 냥이체+난수 도미노), 폭 시스템은 CSS 변수(--lz-*)로 데스크톱(≥721px) 확장. 클로징은 `SeasonCountCta`(기수 카운트)+`BrandCloseV2`. 내비 탭 5개: 함께 읽는 책·모임소개·**진행방식(2026-08-24 부활)**·일정·장소·후기·FAQ. **섹션 순서·배경 — 2026-08-24 전면 재배열 (7섹션 완전 교차, 운영자 "섹션 컬러도 다시 재배열해")**: Hero(+HeroSummary 병치 heroRow) → **함께 읽는 책(B#f0e9e0** — 종이책 조판, 기수 세그 4기·3기·2기·1기**)** → 모임소개(A#f7f3ee, **FeatureQuietSection** 콰이어트 페이드 이어 읽기 — 제목은 2026-08-24부터 '함께 읽는 책' 서식) → **진행 방식(B — `ProcessSection` 독립 섹션, 2026-08-24 5안 확정: 01 자기소개(규칙 원문 모달)·02 오프닝/질문(레이지 노트 4장 모달 — 후기 모달과 동일 UI/UX, 단일 이미지는 넘김 없음)·03 서로의 페이지·04 마무리. 구 HowToBrief(밴드 하단 요약)는 고아 보존)** → 일정·장소(**A** — 6b 괘선 박스+6c 손그림 장소) → **후기(B — 제목 2026-08-24 '멤버들이 남긴 문장'→'후기', ReviewsSection 캐러셀+핀치줌 모달)** → FAQ(**A**) → **ClosingCta+BrandClose(B)**. **섹션 제목 서식은 전 섹션 '함께 읽는 책' 기준으로 통일(2026-08-24)** — 32px/800 좌정렬 + 주황 괘선 56×1.5, ≤480px 26px. 5회차 섹션은 삭제되고 내용은 FAQ 문항으로 이관. About/Closing/Rules/Vibe/**FeatureBoxSection·FifthSessionSection·HowToSection·HowToBrief** 는 **고아 상태로 보존**(렌더 안 함 — 삭제하지 말 것).
- **프리뷰 트리** `app/(main)/lazyday/preview/` : noindex + PreviewBar + 자체 폰트 로드(layout.tsx). 실사이트 컴포넌트의 V2 대응물 + 프리뷰 전용(PhilosophySectionV2, faq-designs). **2026-08-24 아키텍처 재동기화** — 셸(내비·푸터·진행방식·후기·클로징)이 실사이트 것을 **직접 import**(V2 없음): `LandingShell`·`HeroParallax`·`ProcessSection`(구 HowToBrief — 08-24 5안 확정으로 교체)·`ReviewsSection`·`SeasonCountCta`+`BrandCloseV2`. Schedule·모임소개(FeatureQuietSection)만 실↔프리뷰 **분리 사본 쌍**이 유효(§4 참조 — 한쪽 수정 시 쌍도 같은 값으로). 구 `NavBarV2`·`Footer`(구 프리뷰 전용)·`HowToSectionV2`·`ClosingSectionV2`·구 `preview/ReviewsSection`(실물 사진 대기 시절 플레이스홀더)는 실사이트의 대응 고아들과 함께 **고아 보존**(렌더 안 함, 삭제 금지). FifthSession은 섹션 삭제로 실·프리뷰 모두 미렌더(고아 보존). **데스크톱(≥721px) 반응형 훅도 포트 완료**(2026-08-24) — Book/Schedule/FeatureQuietSection(프리뷰)는 자체 CSS 파일(`preview.module.css`·`ScheduleSectionV2.module.css`·`preview/FeatureQuietSection.module.css`)에서 `landing-shell.module.css`가 주입하는 `--lz-carousel-*`·`--lz-cover-*`·`--lz-cal-*`·`--lz-qt-*`·`--lz-rule` 변수를 실사이트와 동일하게 소비하도록 값을 맞췄다(390·1280px 픽셀 동일 실측) — **`--lz-*` 변수를 새로 추가·변경할 땐 이 3개 프리뷰 파일도 같이 확인할 것**(안 하면 프리뷰만 조용히 데스크톱 확장에서 빠진다). **배경 교차도 실사이트와 동일**(2026-08-24 전면 재배열을 쌍 파일·공유 파일이 그대로 전파). 예외 1건: `ScenesSection`(사진 콜라주, 프리뷰 전용 보류, 배경 A)이 후기(B)와 FAQ(A) 사이에 끼어 있는데, 이미 교차 중인 두 색 사이에 실사이트에 없는 섹션 하나를 끼우는 이상 **2색 교차로는 완전한 해소가 수학적으로 불가능**(홀수 삽입) — 나머지 전 구간을 실사이트와 맞추는 대신 Scenes(A)-FAQ(A) 인접 하나만 예외로 남겼다(선례: 2026-07-22~08-17 후기-FAQ B-B 인접도 같은 방식으로 용인). Scenes 실사이트 이식(사진 확보) 시 재검토.
...
| `FaqSection.module.css` | FaqSection, preview/FaqSectionV2, ReviewsSection **(3곳 렌더)** + 구 preview/ReviewsSection(파일은 여전히 import — 고아·미렌더) | **클래스 파티션 주의**: `line*` = 실 FAQ A안 전용 / `.answer`·`.peek*`·`.fade*`·`.moreHint` = 프리뷰 FaqSectionV2 전용(구형, 삭제 금지) / `titleRow`·`sectionTitle` = 공유. 프리뷰는 2026-08-24부터 실사이트 `ReviewsSection` 을 직접 렌더(§2) — 구 preview/ReviewsSection(플레이스홀더)은 고아 |
...
| `NavBar.module.css` | NavBar, preview/NavBarV2 | **둘 다 고아** (2026-08-12 LandingShell 대체 / 2026-08-24 프리뷰도 동기화) — 렌더 안 함, 삭제 금지 |
| `BrandCloseSection.module.css` | BrandCloseSection, preview/ClosingSectionV2 | **둘 다 고아** (2026-08-12 SeasonCountCta+BrandCloseV2 대체 / 2026-08-24 프리뷰도 동기화) — 렌더 안 함, 삭제 금지 |
...
| `FifthSessionSection`·`LandingShell`(landing-shell.module.css)·`ProcessSection`(process-section.module.css — 구 HowToBrief 대체, 08-24)·`ReviewsSection`(ReviewsSection.module.css)·`SeasonCountCta`(season-count-cta.module.css 등)의 module.css | **V2 없음** — 프리뷰 랜딩이 실사이트 컴포넌트를 직접 import (2026-08-24부로 후자 4개 추가). `HowToBrief`(howto-brief.module.css)는 08-24부로 고아 — 렌더 안 함, 삭제 금지 | 수정 = 실사이트+프리뷰 동시 변경. 시안 실험 시 이 파일들 수정 금지, 쇼케이스 전용 CSS를 새로 만들 것 |
| `ScheduleSection`·`FeatureQuietSection` (실) ↔ `ScheduleSectionV2`·`preview/FeatureQuietSection` (프리뷰) | **분리된 파일 쌍** (2026-07-06 배포) | 실·프리뷰가 같은 디자인의 **별도 사본** — 한쪽 수정 시 반드시 쌍도 같은 값으로 (TSX 쌍 동기화 원칙). CSS import 공유 없음. **ScheduleSection 캘린더는 사본이 하나 더**: `apply/ApplyCalendar.tsx/.module.css`(실·프리뷰 apply 공용, 2026-07-27) — 랜딩 캘린더 수정 시 3벌 동기화. ⚠ `HowToSection`·`ClosingCtaSection`(실)과 그 프리뷰 쌍(`HowToSectionV2`·`ClosingSectionV2`)은 2026-08-24부로 **더 이상 유효한 쌍이 아니다** — 실쪽이 먼저 고아가 됐고(2026-08-12·08-17) 프리뷰쪽도 뒤따라 고아 처리됐다(위 행 참조) |
...
- **기수 전환 시**: season-config.ts 수정 + 이 문서와 DECISIONS.md의 기수 표기(현 **4기 모집(open), 기간 9/9–11/1(1회차 수요일 9/9 시작), 마감 9/7 — showDeadline=false로 비표기(경과 시 자동 마감)**, 요일 수→일→화·일요일 오전·오후 2슬롯) 일괄 갱신.
