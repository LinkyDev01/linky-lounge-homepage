 ...
 * 기획서는 `/hosts/apply`(한 화면에 질문 하나, HostPlanFlow). 접수 계약은 v1 과 같다(type:"host").
 *
 * 운영자 지적 3건(09-03 "좋긴 한데")에 대한 응답 — 자세한 근거는 hosts.module.css 머리 주석:
 *   ① 브랜드 문장을 본문에서 뺐다(푸터에 이미 있다 — 반복하면 캐치프레이즈가 된다). 텍스트 대신
 *      **지난 모임 포스터 스트립**(레포 보유 이미지 9장, 전부 4:5)이 "이 이름으로 열린 모임"의 증거가 된다.
 *   ② 안내는 한 화면(세 단락 + 절차 3행)으로 줄이고, 기획서는 `/hosts/apply` 에서 한 화면에 질문 하나씩.
 ...
 * 색인은 레이아웃 호스트 판정(lazy-club.com 만 index), 사이트맵 등재. 진입 링크(내비·푸터·사람 페이지)는 별도 결정.
 */
