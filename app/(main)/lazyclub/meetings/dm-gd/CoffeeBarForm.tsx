 ...
 *
 * **가로는 왕복 한 번뿐**이다 (운영자 2026-08-26 "왕복 2회로 가로축 왕복이 되는데
 * 1회면 충분해. 조금더 줄이거나 더 세로방향으로 쏘면 되겠어"): 가로 초속을 8.14 →
 * **3.4**(이동폭 배수)로 낮추고 세로를 -820 → **-1200px/s** 로 올렸다. 그래서 오른쪽
 * 벽만 한 번 찍고 돌아오며, 남는 에너지는 전부 위아래 반사로 간다 — 느려졌는데도
 * 꼬불꼬불함은 그대로다. 용수철(KS 140·C 14)도 함께 세워 되돌아오는 길이 늘어지지 않게 했다.
 *
 * 실측(1초·60프레임, dx=295px): 최고속 ≈995px/s(직전의 42%) · **오른쪽 벽 1회 ·
 * 왼쪽 벽 0회 · 위아래 벽 11회** · 착지 오차 1px.
 */
