...
/**
 * 이용약관 — 운영자 법률 검토 완료본 그대로 게재 (2026-07-04 반영).
 * 탭 구조·환불 규정 별도 탭은 폐기 — 제3장에 환불 내용이 이미 전부
 * 포함되어 있어 중복 노출 불필요 (운영자 판단).
 * 요약·의역 없이 조문 원문만 게재.
 */
      ...
      <div className={styles.container}>
        ...
        <Article title="제2조 (정의)">
          <ol className={styles.clauseList}>
            ...
            <li>&ldquo;시작일&rdquo;이란 각 회차가 진행되는 날을 말한다. 하나의 회차가 둘 이상의 요일에 걸쳐 진행되는 경우에는 그 중 가장 이른 날을 해당 회차의 시작일로 한다.</li>
            <li>&ldquo;기산일&rdquo;이란 제10조의 환불 기준을 적용하는 기준이 되는 날을 말하며, 회원이 배정된 기수의 정규 1회차 시작일로 한다.</li>
          </ol>
        </Article>
        ...
        <Article title="제4조 (약관 외 준칙)">
          <p className={styles.clause}>
            이 약관에서 정하지 아니한 사항은 관계 법령 및 회사가 별도로 정하는 개별 안내(모임 규칙, 신청 안내 등)에 따른다.
          </p>
        </Article>
        ...
        <Article title="제5조 (서비스의 내용)">
          <ol className={styles.clauseList}>
            ...
            <li>회사는 회원 간 대화의 질을 유지하기 위하여 인터뷰를 거쳐 회원을 모집한다.</li>
            <li>회사는 신규 회원이 본인의 기수가 시작되기 전 직전 기수의 자유 독서모임에 사전참여할 수 있는 절차를 운영할 수 있다.</li>
            <li>하나의 회차는 복수의 요일에 걸쳐 진행될 수 있으며, 회원은 회차마다 그 중 하나의 요일을 선택하여 참석한다.</li>
            <li>특정 요일에 진행되는 모임의 참가 인원이 2명 이하인 경우, 회사는 해당 요일의 모임 진행을 취소하고 회원에게 같은 회차의 다른 요일 모임 참석을 안내할 수 있다. 다만 각 회차의 마지막 요일에 진행되는 모임은 참가 인원과 관계없이 진행한다. 회원이 안내된 다른 요일 모임에 참석할 수 없는 경우에는 해당 회차에 한하여 제12조를 준용한다.</li>
          </ol>
        </Article>

        <Article title="제6조 (이용계약의 성립)">
          <ol className={styles.clauseList}>
            <li>이용계약은 이용희망자가 인터뷰에 참여하고, 회사가 이용신청을 승낙하며, 이용희망자가 참가비를 결제함으로써 성립한다.</li>
        ...
        <Article title="제7조 (회원의 의무)">
          <ol className={styles.clauseList}>
            ...
            <li>회원은 모임 중 알게 된 다른 회원의 개인정보 및 발언 내용을 본인의 동의 없이 외부에 공개하거나 모임 목적 외로 사용해서는 아니 된다.</li>
          </ol>
        </Article>

        <Article title="제8조 (회사의 의무)">
          <ol className={styles.clauseList}>
            <li>회사는 북클럽을 안정적으로 운영하기 위하여 노력하며, 진행자 사정으로 모임 진행이 어려운 경우 이를 사전에 회원에게 안내한다.</li>
        ...
        <Article title="제9조 (청약철회)">
          <ol className={styles.clauseList}>
            <li>회원은 참가비를 결제한 날부터 7일 이내에는 위약금이나 수수료 없이 청약철회를 할 수 있다. 이 경우 아직 제공되지 아니한 회차분의 참가비는 전액 환불한다.</li>
            <li>제1항의 기간 중이라도 이미 시작된 회차가 있는 경우, 북클럽은 그 회차의 공급에 실제로 든 비용에 상당하는 금액을 공제한 나머지를 환불할 수 있다.</li>
          ...
          <ol className={styles.clauseList}>
            <li>
              ...
              <ol className={styles.itemList}>
                <li>기산일 전까지 취소하는 경우, 참가비 총액의 60퍼센트를 환불한다.</li>
                <li>정규 1회차 시작일이 지난 후부터 정규 2회차 시작일 전까지 취소하는 경우, 참가비 총액의 40퍼센트를 환불한다.</li>
                <li>정규 2회차 시작일이 지난 후부터 정규 3회차 시작일 전까지 취소하는 경우, 참가비 총액의 20퍼센트를 환불한다.</li>
                <li>정규 3회차 시작일이 지난 후부터는 정규 4회차 및 자유 독서모임을 포함하여 환불하지 아니한다.</li>
              </ol>
            </li>
            <li>환불률은 회원이 실제로 환불을 신청한 시점을 기준으로 적용한다. 별도의 신청 없이 특정 회차에 불참한 경우는 환불 신청으로 보지 아니한다.</li>
          </ol>
        </Article>

        <Article title="제11조 (사전참여자에 대한 특칙)">
          <ol className={styles.clauseList}>
            <li>제2조 제8항에도 불구하고, 사전참여를 신청하여 직전 기수의 자유 독서모임에 참석한 회원의 기산일은 그 자유 독서모임의 시작일로 한다.</li>
            <li>
              사전참여자에 대하여는 제9조 제1항의 기간이 지난 이후의 환불에 관하여 제10조 제1항을 적용하지 아니하고 다음 각 호에 따른다.
              <ol className={styles.itemList}>
                <li>기산일 전까지 취소하는 경우, 참가비 총액의 60퍼센트를 환불한다.</li>
                <li>기산일이 지난 후부터 정규 1회차 시작일 전까지 취소하는 경우, 참가비 총액의 40퍼센트를 환불한다.</li>
                <li>정규 1회차 시작일이 지난 후부터 정규 2회차 시작일 전까지 취소하는 경우, 참가비 총액의 20퍼센트를 환불한다.</li>
                <li>정규 2회차 시작일이 지난 후부터는 정규 3회차, 정규 4회차 및 자유 독서모임을 포함하여 환불하지 아니한다.</li>
              </ol>
            </li>
          </ol>
        </Article>
        ...
        <Article title="제13조 (환불 신청 및 처리)">
          <ol className={styles.clauseList}>
            <li>환불 신청은 북클럽이 안내하는 연락처로 문자 또는 카카오톡을 통하여 접수한다.</li>
            <li>환불은 접수일로부터 영업일 기준 5일 이내에 처리한다.</li>
            <li>이 약관에 따라 산정된 금액 외에 별도의 위약금이나 수수료는 청구하지 아니한다.</li>
          </ol>
        </Article>

        <h2 className={styles.chapterTitle}>제4장 기타</h2>

        <Article title="제14조 (이용제한 및 자격상실)">
          <ol className={styles.clauseList}>
        ...
        <Article title="제15조 (개인정보의 보호)">
          <p className={styles.clause}>
            회사는 회원의 개인정보를 관계 법령 및 회사가 별도로 게시하는 개인정보처리방침에 따라 수집, 이용, 보관한다.
          </p>
        </Article>

        <Article title="제16조 (손해배상 및 면책)">
          <ol className={styles.clauseList}>
            <li>회사는 천재지변, 그 밖의 불가항력적 사유로 서비스를 제공할 수 없는 경우 그 책임을 지지 아니한다.</li>
            <li>회사는 회원 간에 발생한 분쟁에 대하여 고의 또는 중대한 과실이 없는 한 책임을 지지 아니한다.</li>
          </ol>
        </Article>

        <Article title="제17조 (분쟁해결 및 관할법원)">
          <ol className={styles.clauseList}>
            <li>회사와 회원은 북클럽 이용과 관련하여 발생한 분쟁을 원만하게 해결하기 위하여 성실히 협의한다.</li>
            <li>협의에도 불구하고 분쟁이 해결되지 아니하는 경우, 민사소송법상의 관할법원에 소를 제기할 수 있다.</li>
          </ol>
        </Article>
