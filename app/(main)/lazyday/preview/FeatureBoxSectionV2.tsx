...
/**
 * 모임 소개 — 상호작용은 원본과 동일한 '페이드 + ...더보기' 형태 유지
 * (운영자 결정 2026-07-03: '요약+자세히 보기' 방식보다 기존 형태 선호)
 * 원본 대비 변경점은 '불균형의 균형' 카드 추가 하나뿐.
 */
const items: { label: ReactNode; paragraphs: ReactNode[]; note?: string }[] = [
  ...
  },
  {
    ...
    paragraphs: [
      <>비슷한 결을 가진 사람들이 모였다고 해서 같은 결론에 도달할 필요는 없거든요. 같은 곳에서 멈추는 사람들이라도 거기서 자라난 <span className={styles.accent}>사유의 궤적</span>은 각자 다르니까요.</>,
      <>바우하우스의 정갈한 비대칭처럼, 각기 다른 궤적을 그려온 사람들의 단련된 사유가 거칠게 부딪힐 때 그 <span className={styles.accent}>불협화음</span>이 오히려 고전의 본질을 꿰뚫는 하나의 선율이 되는 순간이 있어요.</>,
      <>다 같이 고개 끄덕이는 무색무취한 공감 말고, 각자의 뚜렷한 철학을 바탕으로 <span className={styles.accent}>사유의 밀도</span>를 높일 수 있는 자리, 그 부조화 속에서 이전에 없던 지적 조화를 발견하는 자리. 그게 레이지데이가 만들고 싶은 자리예요.</>,
    ],
  },
  {
  ...
  },
  {
    ...
    paragraphs: [
      <>레이지데이가 직접 운영하는 약 30평 규모의 링키라운지에서 진행해요. <span className={styles.accent}>사당역 10번 출구</span>에서 4분 거리예요.</>,
