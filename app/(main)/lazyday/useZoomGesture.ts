...
/**
 * 사진 뷰어용 핀치 줌·팬 엔진 (2026-08-19, 후기 모달 확대 개선).
 *
 * ⚠ preview/useZoomGesture.ts 와의 **분리 사본 쌍** — 실사이트는 preview/ 를
 *   import 하지 않는다는 규칙(§1)에 따라 값 그대로 복사했다. 한쪽을 고치면
 *   반드시 쌍도 같은 값으로 맞출 것 (검수대: preview/reviews-zoom-check).
 *
...
/** 더블탭·클릭 1회로 뛰는 목표 배율 — 첫 확대가 과하다는 지적(2026-08-19)으로 2.5→1.8 */
const DOUBLE_TAP_SCALE = 1.8
/** +/− 버튼 1클릭당 배율 증감분 — 같은 지적으로 0.6→0.35, 더 세밀하게 조절되게 */
...
export function useZoomGesture() {
  ...
  const stepZoom = useCallback(
    (dir: 1 | -1) => {
      ...
      if (!frameEl) return
      const rect = frameEl.getBoundingClientRect()
      const next = clamp(xfRef.current.scale + dir * ZOOM_STEP, MIN_SCALE, MAX_SCALE)
