...
| **편집기에서 직접 고친 내용을 덮어쓴다** (§6 "실배포본이 미러보다 최신일 수 있다") | push 전 기준선 검사 — 실배포본이 **마지막으로 배포한 커밋** 시점의 레포와 다르면 멈추고 `pull` 을 안내. 그 커밋은 배포 설명(`auto: <SHA> …`)에서 읽는다 — 배포 자체가 원장이다 (아래 참고) |
...
### 기준선은 "마지막으로 배포한 커밋"이다 (2026-08-25 수정)

종전에는 기준선을 `HEAD~1`(또는 push 의 `event.before`)로 **추정**했다. 그 추정은
**GAS 커밋 뒤에 다른 커밋이 하나라도 얹히면 무너진다** — HEAD~1 이 이미 신코드라
실배포(구코드)와 달라져 가짜 드리프트로 배포가 막힌다. 8/23~8/25 배포가 막힌 사이
문서 커밋이 얹혀 실제로 이 함정을 밟았다.

이제는 `cmdDeploy` 가 배포 설명에 적어 두는 `auto: <짧은SHA> <커밋 제목>` 을 읽어
**그 커밋**을 기준선으로 쓴다. 배포가 곧 "우리가 무엇을 올렸는가"의 원장이므로 추정이
필요 없다. 설명이 `auto:` 로 시작할 때만 신뢰하고(운영자가 `-m` 으로 쓴 설명은 SHA 가
아니다), 그 커밋이 클론에 없으면 종전 방식(`--baseline` → `HEAD~1`)으로 후퇴한다.

⚠ 그래서 워크플로는 **전체 이력**을 받는다(`fetch-depth: 0` + `filter: blob:none`).
마지막 배포가 얼마나 과거인지 알 수 없기 때문이다 — 8/25 실측으로 142 커밋 전이었다.
`blob:none` 이라 과거 blob 은 필요할 때만 받으므로 종전 `depth: 50` 보다 오히려 가볍다.
