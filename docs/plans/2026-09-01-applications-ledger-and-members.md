...
1. **`profiles` 마이그레이션(다음 번호 = 0010 — 0006~0009 는 사용됨, 위 불변 원칙 5 참조)** — 테이블 + **`drop trigger if exists`/`create trigger` set_updated_at**(초안은 updated_at 을 선언만 하고 트리거를 안 걸어 값이 고정됐다) + **실행 가능한 SQL 로 RLS**(초안은 주석이라 **RLS 꺼진 채 이메일이 담길 뻔했다**) + `comment on`.
...
⚠ Supabase MCP 게이트는 **2026-09-01 승인·실사용으로 해소** — 다만 마이그레이션이 있는 Phase(P2.5 의 admin 라우트는 무관, P4a 의 0010)는 세션마다 연결 여부를 다시 확인하고 끊겨 있으면 운영자에게 알리고 대기(불변 원칙 4).
