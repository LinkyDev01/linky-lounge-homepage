// ============================================================
// ⚠️ 미실행 초안 — **배포하지 말 것** (실사 2026-08-18)
// ============================================================
// 2026-07-23 임포트 이후 한 번도 실행된 적이 없고, 대상 스키마(supabase/schema.sql)
// 자체가 정본이 아니다 (그 파일 헤더 참조 — 보존기간·RLS·FK 문제).
//
// ⛔ 이 스크립트는 service_role 키를 **소스에 평문으로** 채우게 설계돼 있다
//    (아래 SB_KEY). 이는 CLAUDE.md §6 "비밀값은 코드가 아닌 스크립트 속성" 규율과
//    정면으로 충돌하며, 지금은 gas/ 변경이 **자동으로 실배포**되는 파이프라인
//    (.github/workflows/gas-deploy.yml)이 붙어 있어 키를 채워 커밋하면 그대로 나간다.
//    실제 이관을 하게 되면 PropertiesService 로 옮겨 다시 쓸 것. 지금 상태로 두되
//    **플레이스홀더를 실제 키로 바꾸지 않는다.**
// ============================================================
